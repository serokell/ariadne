module Ariadne.Wallet.Cardano.Kernel.DB.Read (
...
import Ariadne.Wallet.Cardano.Kernel.DB.BlockMeta (AddressMeta)
import Ariadne.Wallet.Cardano.Kernel.DB.HdWallet
  (HdAccountId, HdAddress, HdWallets)
import Ariadne.Wallet.Cardano.Kernel.DB.HdWallet.Read
  (HdQueryErr, readAddressesByAccountId, readHdAccountCurrentCheckpoint)
import Ariadne.Wallet.Cardano.Kernel.DB.Spec (accCheckpointAddressMeta)
...
readAddressMeta snapshot accountId cardanoAddress
    = view (accCheckpointAddressMeta cardanoAddress) (walletQuery' snapshot checkpoint)
    where
