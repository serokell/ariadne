...
equivalentT activeWallet esk = \mkWallet w ->
  ...
  where
    ...
    walletBootT ctxt utxo = do
        res <- liftIO $ Kernel.createWalletHdSeq passiveWallet
                                                 (Kernel.HasNonemptyPassphrase False)
                                                 walletName
                                                 assuranceLevel
                                                 esk
                                                 utxoByAccount
        case res of
             ...
             Right hdRoot -> do
                 ...
                 liftIO $ Keystore.insert (WalletIdHdSeq $ hdRoot ^. HD.hdRootId) esk keystore
