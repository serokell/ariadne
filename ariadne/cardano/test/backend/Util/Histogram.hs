...
import Prelude hiding (empty, max, readFile, toList, writeFile)
import qualified Prelude

import qualified Data.IntMap.Strict as Map
import qualified Data.Set as Set
import qualified Data.String as Base (lines)
...
readFile binSize fp = parse <$> IO.readFile fp
  where
...
nLargestBins p (Histogram bz m) = Histogram bz (aux m)
  where
    allBins  = sortBy (flip (comparing snd)) $ toPairs m
