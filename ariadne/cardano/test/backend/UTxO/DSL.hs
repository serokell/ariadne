{-# LANGUAGE DeriveAnyClass, OverloadedStrings, UndecidableInstances #-}
...
import Prelude hiding (foldr, sum, toList)
...
import Ariadne.Wallet.Cardano.Kernel.Util (at, restrictKeys, withoutKeys)
...
ledgerTails (Ledger (NewestFirst l)) =
    zipWith (\t ts -> (t, Ledger (NewestFirst ts))) l (Unsafe.tail (tails l))
...
utxoBalance = sum . map outVal . elems . utxoToMap
