{-# LANGUAGE GeneralizedNewtypeDeriving, ImplicitParams, InstanceSigs,
...
import Ariadne.Wallet.Cardano.Kernel.Util (at)

import Pos.Block.Logic (RawPayload(..), createMainBlockPure)
import Pos.Client.Txp
import Pos.Core
import Pos.Core.Block
  (Block, BlockHeader(..), GenesisBlock, MainBlock, gbHeader, genBlockLeaders,
  mkGenesisBlock)
import Pos.Core.Chrono
import Pos.Core.Delegation (DlgPayload(..))
import Pos.Core.Genesis
  (GenesisWStakeholders, gdBootStakeholders, gdProtocolConsts,
  genesisProtocolConstantsToProtocolConstants)
import Pos.Core.Txp (TxAux(..), TxId, TxIn(..), TxOut(..), TxOutAux(..))
import Pos.Crypto
import Pos.Lrc (followTheSatoshi)
import Pos.Ssc (defaultSscPayload)
import Pos.Txp (Utxo, txOutStake)
import Pos.Update
...
updateStakes gs (ResolvedBlock txs _) =
    composeM $ map go txs
  where
    composeM :: forall n a. Monad n => [a -> n a] -> a -> n a
    ...
    go (ResolvedTx ins outs) =
      ...
      where
        ...
        addStakes = concatMap txOutStake' $ toPairs (_fromDb outs)
